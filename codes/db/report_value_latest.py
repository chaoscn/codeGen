tableReportValueLatest={
    "tableName": "report_value_latest",
    "columns": (
    ...
    "indexs": (
        { "indexName": "report_id_server_id", "unique": True, "columns": ("i_report_id", "i_server_id", ) },
        ),
    }
tables.append(tableReportValueLatest)

tableReportValueLatest["dataObjectExtra"]="""
...
"""
tableReportValueLatest["sps"]=(
    {
        "spName": "GetReportValueLatestsByReportID",
        "description": "Get report value latests by report ID",
...
"""SELECT
    ...
    FROM report_value_latest WITH (FORCESEEK)
    WHERE i_report_id             = @i_report_id
    
    CHKERR({ERROR_GET_REPORT_VALUE_LATESTS_BY_REPORT_ID})
    RETURN 0""",
    },
    {
        "spName": "GetReportValueLatestsByReportIDAndServerIDs",
        "description": "Get report value latests by report ID and server IDs",
        "inputParameters":
"""@i_report_id           int,
...
"""CHECK_NOTNULL(@vc_server_ids)
    ...
    CHKERR({ERROR_GET_REPORT_VALUE_LATESTS_BY_REPORT_ID_AND_SERVER_IDS})
    RETURN 0""",
    },
    {
        "spName": "DeleteReportValueLatestsByReportID",
        "description": "Delete report value latests by report ID",
        "inputParameters": """@i_report_id           int""",
        "sqlQuery":
"""TRANINIT

    DELETE FROM report_value_latest
    WHERE i_report_id = @i_report_id
    
    SELECT @@ROWCOUNT AS 'rowcount'
    
    TRANCHKERR({ERROR_DELETE_REPORT_VALUE_LATESTS_BY_REPORT_ID})
    TRANRETURN""",
    },
    {
        "spName": "DeleteReportValueLatestsByServerID",
        "description": "Delete report value latests by server ID",
...
"""TRANINIT
    ...
    TRANCHKERR({ERROR_DELETE_REPORT_VALUE_LATESTS_BY_SERVER_ID})
    TRANRETURN""",
    },
    {
        "spName": "InsertOrUpdateReportValueLatest",
        "description": "Insert or update report value latest",
        "inputParameters":
"""@i_report_id                 int,
    @i_server_id                 int,
...
"""DECLARE @bi_sequence_id int
    ...
    IF (@bi_sequence_id = 0)
    BEGIN
        INSERT INTO report_value_latest
        (
            i_report_id,
            i_server_id,
            ti_current_hour,
    ...
    TRANCHKERR({ERROR_INSERT_OR_UPDATE_REPORT_VALUE_LATEST})
    TRANRETURN""",
    },
    )