...
WHILE (@reportID <= 2)
BEGIN
    SET @hourlyPartitionID = @reportID - 1
    SET @serverID = 5
    WHILE (@serverID <= 322)
    BEGIN
        SET @currentDay = 0
        WHILE (@currentDay <= 30)
        BEGIN
            SET @dateTime     = DATEADD(dd, @currentDay - 31, CONVERT(date, GETDATE()))
            ...
            EXEC spm_InsertOrUpdateReportValueHourly{}VERSION_SUFFIX
                @i_hourly_partition_id       = @hourlyPartitionID,
